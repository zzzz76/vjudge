 ...
 * @author zzzz76
 */
public class ECNCaptchaRecognizer {
    private static Tesseract tesseractNum;
    private static Tesseract tesseractOpe;

    static {
        tesseractNum = new Tesseract();
        tesseractNum.setDatapath("/home/zzzz76/IdeaProjects/vjudge/web/tessdata"); // 这个数据集竟然更好
    ...
    static {
        ...
        tesseractOpe.setDatapath("/home/zzzz76/IdeaProjects/vjudge/web/tessdata");
    ...
    public static void printok(DedicatedHttpClient client) {
        System.err.println(client.getContext());
    }

    public static void getCaptcha(DedicatedHttpClient client) throws IOException {
        // 获取关键的task key
        String result = client.get("/captcha/refresh/", HttpStatusValidator.SC_OK).getBody();
        String key = Tools.regFind(result, "\"key\": \"([\\s\\S]*?)\",");

        // 对图片进行简单打印
        printImage(modelFiltering(refreshCaptcha(client, key)));

        // 同一个验证码进行三次刷新, 提高识别率, 期间可以加入一些特征
        int[][] hashtables = new int[3][11];
        for (int j = 0; j < 3; j++) {
            BufferedImage image = refreshCaptcha(client, key);
            List<BufferedImage> arrayList = splitImage(image);
            ...
            if (h == 42) {
                // plus
                hashtables[1][3]++;
                if (w > 154) hashtables[0][10]++;
                else hashtables[0][doOCRToNum(arrayList.get(0))]++;
            } else {
                // minus and times
                hashtables[1][doOCRToOpe(arrayList.get(1))]++;
                if (w > 176) hashtables[0][10]++;
                else hashtables[0][doOCRToNum(arrayList.get(0))]++;
            }
            hashtables[2][doOCRToNum(arrayList.get(2))]++;
        }
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (j == 1) {
                System.out.println(operationList[index]);
            } else {
                if (hashtables[j][4] > 0) index = 4;
                if (hashtables[j][8] > 0) index = 8;
                if (hashtables[j][7] > 0) index = 7;
    ...
    public static BufferedImage refreshCaptcha(DedicatedHttpClient client, String key) throws ClientProtocolException, IOException {
        // 这里的key如果是url就更好了
        // 这里实际上用一个客户端不断的刷新页面
        HttpGet get = new HttpGet("/captcha/image/" + key);
        BufferedImage image = client.execute(get, new ResponseHandler<BufferedImage>() {
            ...
            public BufferedImage handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
                ...
                try {
                    File captchaImg = File.createTempFile("ECN", ".png");
    ...
    private static int levenshtein(String str1, String str2) {
        ...
        for (int y = 1; y < h; y++) {
            for (int x = 1; x < w; x++) {
                ...
                matrix[y][x] = Math.min(Math.min(temp, matrix[y][x - 1] + 1), matrix[y - 1][x] + 1);
            }
        }
